...
**Active milestone: M6 — Pilot Readiness.**
**Active branch:** `feature/m6-pilot-readiness` (not created yet — that's this session's step 0).
...
M5 — Profile & Settings is complete (all 4 issues Done, retro filed as FRE-354). Notably: `index.html`'s hardcoded `lang="nl"` never followed the in-app language toggle — `i18n/index.js` now syncs `document.documentElement.lang` on init and on every `setLang()` call, no call-site changes needed (FRE-326); `ProfileView`'s address preview hardcoded "Reyer Anslostraat {house_number}" instead of deriving the street name — now threaded through as a `streetName` prop from the `streetInfo` already being fetched in `App`, plus `AdminView`'s `nr. X` house-number prefixes and role-`<select>` options (`Bewoner`/`Moderator`/`Admin`) swept into existing/new i18n keys (FRE-327); three independently-drifted ad hoc `<div onClick>` toggle switches (`ProfileView`'s notification toggles, `AttendanceToggle`'s RSVP switch, `DesignPage`'s design-system demo) were consolidated into one shared `components/Switch.jsx` — a real `<button role="switch" aria-checked>` that gets keyboard support for free from native button semantics, with the adjacent language picker converted to `<button aria-pressed>` since it's a different (two-way-choice, not on/off) pattern (FRE-328); and a frontend-only smoke test (`App.smoke.profile.test.jsx`, following M2's RTL pattern rather than M3/M4's Postgres pattern since nothing backend changed) covers profile data loading plus both a notification-toggle and a language-switch surviving a simulated reload (FRE-329). Merged to `main` via a regular merge commit (`768858a`); one CI flake hit on `main` (`build-backend` failed pushing to GHCR with a transient "unknown blob" registry error, unrelated to any code change — the image itself built fine) and cleared on retry. Production smoke test confirmed clean by Wendy on streetfeed.nl (profile data, notification-toggle persistence, language-switch persistence including `<html lang>`); the fourth check (keyboard-only tab-through of the toggles) wasn't run since Wendy currently tests only on mobile, where that interaction model doesn't really apply — accepted as sufficient given the native-button keyboard semantics are already covered by FRE-328's automated a11y check.
...
1. Fetch M6's open issues from Linear (project "Streetfeed v1.0", milestone "M6 — Pilot Readiness"). Work them one at a time.
...
3. When every M6 issue is closed: file one `Retro: M6 — Pilot Readiness` issue in that milestone using this template:
   ```
   - What went well
...
5. Once Wendy confirms the production smoke test is clean: this is the last milestone (M6 — Pilot Readiness), so instead of advancing to a next milestone, treat this as v1.0 pilot-ready — confirm with Wendy how she wants this file closed out (e.g. a final "v1.0 shipped" note) rather than assuming a next milestone exists. Then stop and tell the user to `/clear`.
...
## Workflow notes learned during M5 (worth keeping)

- Pre-audit issue descriptions with hardcoded line numbers (e.g. "App.jsx:1556-1558") age fast in a codebase under active refactor — M2/M3's extraction work shrank `App.jsx` from ~2149 to ~927 lines between when these issues were filed and when they were worked, so every issue needed a quick re-grep for the actual current location before starting. Don't trust cited line numbers at face value; verify against current code first.
- When two issues touch overlapping regions of the same file (FRE-327 and FRE-328 both edited `ProfileView` in `App.jsx`), a clean per-issue commit split is still achievable without blocking on sequencing: implement both, then temporarily revert one issue's hunks with Edit, commit the other issue's changes alone, reapply the reverted hunks, and commit those. Keeps the per-issue commit history honest (matches this project's established convention) even when the actual edits were made together.
- "Fix once and reuse" for duplicated UI patterns (here, three independently-drifted toggle-switch implementations) doesn't mean forcing every call site to look identical — a shared component can stay parameterized (size, colors) to preserve each site's existing visual intent while still centralizing the actual fix (real `<button role="switch">` semantics, in this case).
- Not every "toggle-looking" control is actually a switch — a two-way exclusive choice (like the NL/EN language picker sitting right next to the notification toggles in the same issue's description) is a different a11y pattern (`aria-pressed` buttons, not `role="switch"`/`aria-checked`) and shouldn't be forced into a binary Switch component just because it was described alongside one.
- A `build-backend` CI failure with `ERROR: unknown blob` during `docker push` to GHCR (after the image itself built successfully) is a transient registry-push glitch, not a real problem — confirmed by the failure occurring on a merge that only touched frontend files. `gh run rerun <id> --failed` re-ran just that job and it passed immediately. Worth checking "did the image build, or did the push fail" before assuming a real regression when `build-*` jobs fail on `main`.
- Manual smoke-test checklists for keyboard-only interaction can't be completed by a Product Owner who only tests on mobile — that's an expected, acceptable gap (the interaction model doesn't apply on a phone), not a blocker, especially when the underlying semantic guarantee (native `<button>` keyboard support) is already covered by an automated test.
